...
void Options::define(const char* aDefinition) {
   ...
   strcpy_s(tempstr, len + 1, definitionString);

   char *optionName, *nextToken1, *nextToken2;   // option name to add to list

   // get the first option name
   optionName = strtok_s(tempstr, " \t\n|", &nextToken1);
   if (optionName[0] == '=') {
...
const char* Options::getCommandLine(void) {
   ...
   strcpy_s(commandString,length + 1, gargv[0]);
   for (i=1; i<gargc; i++) {
      strcat_s(commandString, length, " ");
	  strcat_s(commandString, length, gargv[i]);
   }
...
void Options::verify(int error_check, int suppress) {
   ...
   strcpy_s(tempargument, strlen(gargv[0]) + 1, gargv[0]);
   ...
   while (gargp < gargc && optionend == 0) {
      if (optionQ(gargv[gargp], gargp)) {
         ...
         if ( strlen(gargv[gargp]) == 2 && gargv[gargp][0] == getFlag() &&
			...
			strcpy_s(tempargument, strlen(gargv[gargp]) + 1, gargv[gargp]);
            argument.append(tempargument);
            gargp++;
         }
      }
