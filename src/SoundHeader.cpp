...
SoundHeader::SoundHeader(SoundHeader& aHeader) {
   ...
   strcpy_s(commentString, commentLength + 1, aHeader.commentString);
...
SoundHeader& SoundHeader::operator=(const SoundHeader& h) {
   ...
   commentString = new char[commentLength+1];
   strcpy_s(commentString, commentLength + 1, h.commentString);
...
void SoundHeader::setComment(const char* aString) {
   ...
   commentString = new char[commentLength+1];
   strcpy_s(commentString, commentLength + 1, aString);
}
...
void SoundHeader::setHeader(char* filename) {
...
void SoundHeader::setHighMono(char* comment) {
   setSrate(44100);
   setChannels(1);
   setComment(comment);
...
void SoundHeader::setHighStereo(char* comment) {
   setSrate(44100);
   setChannels(2);
   setComment(comment);
   setNextFormat(SND_FORMAT_LINEAR_16);
...
FileIO& operator<<(FileIO& output, SoundHeader& header) {
   switch (header.getOutputType()) {
      ...
      case TYPE_SND:
         ...
         break;
      case TYPE_WAV_PCM: 
         output.writeLittleEndian((ulong)0x52494646);      // "RIFF"
...
int SoundHeader::getFormat(const char* string) {
   ...
   strcpy_s(extension, 1000, &string[index]);
...
void SoundHeader::processInWavFormat(const char *filename) {
   ...
   sndfile.readBigEndian(tempShort);
   if (tempShort != 1) {
